...
const form = reactive({ kualitas: '' });
const formErrors = ref({});

function buildUrl(template, kbid) {
    return template.replace('__kbid__', kbid);
}
...
async function loadData() {
    ...
    try {
        ...
        errorMessage.value = 'Gagal memuat data kualitas bibit.';
    } finally {
        loading.value = false;
    }
}
...
async function submitForm() {
    ...
    try {
        if (editingKbid.value) {
            await http.put(buildUrl(props.updateUrlTemplate, editingKbid.value), form);
        } else {
        ...
        if (e.status === 422 && e.body?.errors) {
            ...
            errorMessage.value = 'Gagal menyimpan data kualitas bibit.';
        }
    }
}

async function nonaktifkan(item) {
    if (!confirm(`Nonaktifkan kualitas "${item.kualitas}"?`)) return;
    try {
        await http.patch(buildUrl(props.toggleUrlTemplate, item.kbid));
        await loadData();
    } catch (e) {
        errorMessage.value = e.body?.message ?? 'Gagal menonaktifkan kualitas bibit.';
    }
}
...
<template>
    <div class="space-y-6">
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    <TableEmpty v-else-if="items.length === 0" :colspan="2">
                        Belum ada data kualitas bibit.
                    </TableEmpty>
                    <TableRow v-for="item in items" :key="item.kbid">
                        <TableCell>
                            <div class="flex items-center gap-3">
                                <Avatar class="size-9 rounded-lg">
                                    <AvatarFallback :class="['rounded-lg font-semibold', colorFor(item.kualitas)]">
                                        {{ initialOf(item.kualitas) }}
                                    </AvatarFallback>
                                </Avatar>
        ...
        <Dialog v-model:open="dialogOpen">
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{{ editingKbid ? 'Edit Kualitas Bibit' : 'Tambah Kualitas Bibit' }}</DialogTitle>
                </DialogHeader>
                <form class="space-y-4" @submit.prevent="submitForm">
                    <div class="space-y-2">
                        <Label for="kualitas">Kualitas</Label>
                        <Input id="kualitas" v-model="form.kualitas" placeholder="Standard, Premium, Signature, Super Premium" />
                        <p v-if="formErrors.kualitas" class="text-sm text-destructive">
                            {{ formErrors.kualitas[0] }}
                        </p>
                    </div>
                    <DialogFooter>
